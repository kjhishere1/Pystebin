...
haste.prototype.newDocument = function(hideHistory) {
  ...
  if (!hideHistory) {
    window.history.pushState(null, this.appName, subfolder + '/');
  }
  this.setTitle();
  this.lightKey();
...
haste.prototype.lockDocument = function() {
  ...
  this.doc.save(this.$textarea.val(), function(err, ret) {
    if (err) {
      ...
      window.history.pushState(null, _this.appName + '-' + ret.key, subfolder + file);
      _this.fullKey();
      _this.$textarea.val('').hide();
      _this.$box.show().focus();
      _this.addLineNumbers(ret.lineCount);
...
haste.prototype.configureButtons = function() {
  ...
  this.buttons = [
    ...
    },
    {
      ...
      action: function() {
        window.location.href = subfolder + '/raw/' + _this.doc.key;
      }
    },
    {
      $where: $('#box2 .twitter'),
